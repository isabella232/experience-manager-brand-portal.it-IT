...
L’articolo descrive come pubblicare i predefiniti per immagini, gli schemi di metadati e i facet di ricerca personalizzati dall’istanza AEM Author al portale del marchio. La funzione di pubblicazione consente alle organizzazioni di riutilizzare i predefiniti per immagini, gli schemi di metadati e i facet di ricerca creati/modificati nell’istanza AEM Author, riducendo in tal modo le operazioni duplicate.

>[!NOTE]
>
>La possibilità di pubblicare i predefiniti per immagini, lo schema di metadati e i facet di ricerca dall’istanza AEM Author al Portale del marchio è disponibile a partire da AEM 6.2 SP1-CFP7 e AEM 6.3 SP 1-CFP 1 (6.3.1.1).

## Pubblicare i predefiniti per immagini su Brand Portal {#publish-image-presets-to-brand-portal}

I predefiniti per immagini sono una serie di comandi di ridimensionamento e formattazione applicati all’immagine al momento della distribuzione. I predefiniti per immagini possono essere creati e modificati in Brand Portal. In alternativa, se l’istanza AEM Author è in esecuzione in modalità per contenuti multimediali dinamici, gli utenti possono creare dei predefiniti in corrispondenza del AEM Author e pubblicarli AEM Assets Portale marchio, evitando di ricreare gli stessi predefiniti in Brand Portal.\
...
>[!NOTE]
>
>Se l’istanza AEM Author non è in esecuzione in modalità **** Dynamic Media (il cliente non ha acquistato Dynamic Media), la rappresentazione TIFF **** piramidale delle risorse non viene creata al momento del caricamento. I predefiniti per immagini o le rappresentazioni dinamiche funzionano su TIFF **** piramidale di una risorsa, pertanto se il file TIFF **** piramidale non è disponibile nell’istanza AEM Author, non è disponibile anche sul Portale marchio. Di conseguenza, nella barra delle rappresentazioni della pagina dei dettagli della risorsa e nella finestra di dialogo di download non sono presenti rappresentazioni dinamiche.

Per pubblicare i predefiniti per immagini in Brand Portal:

1. Nell’istanza AEM Author, toccate o fate clic sul logo AEM per accedere alla console di navigazione globale, quindi toccate o fate clic sull’icona Strumenti e selezionate **[!UICONTROL Risorse > Predefiniti]** immagine.
...
1. Nell’istanza AEM Author, tocca o fai clic sul logo AEM per accedere alla console di navigazione globale, quindi tocca o fai clic sull’icona **[!UICONTROL Strumenti]** e passa a **[!UICONTROL Risorse > Predefiniti]** immagine.
...
>[!NOTE]
>
>Se a una cartella è applicato uno schema sull’istanza AEM Author, lo stesso schema deve esistere anche nel Portale marchio per mantenere la coerenza nella pagina delle proprietà delle risorse sul portale AEM Author e marchio.

Per pubblicare uno schema di metadati dall’istanza AEM Author al Brand Portal:

1. Nell’istanza AEM Author, tocca o fai clic sul logo AEM per accedere alla console di navigazione globale, quindi tocca o fai clic sull’icona Strumenti e passa a **[!UICONTROL Risorse > Schemi]** di metadati.
...
Se sei disposto a utilizzare un modulo di ricerca personalizzato **[!UICONTROL Risorse Admin Search Rail]** dall’istanza AEM Author, invece di ricreare lo stesso modulo su Brand Portal puoi pubblicare il modulo di ricerca personalizzato dall’istanza AEM Author al Brand Portal.

>[!NOTE]
>
>Per pubblicare su Brand Portal non è possibile pubblicare su **[!UICONTROL Assets Admin Search Rail]** sui AEM Assets i moduli di ricerca bloccati a meno che non vengano modificati. Una volta modificato e pubblicato in Brand Portal, questo modulo di ricerca sostituisce il modulo di ricerca sul Brand Portal.

Per pubblicare il facet di ricerca modificato dall’istanza AEM Author al Brand Portal:
...
1. Nell’istanza AEM Author, toccate o fate clic sul logo AEM per accedere alla console di navigazione globale, quindi toccate o fate clic sull’icona Strumenti e selezionate **[!UICONTROL Generale > Moduli]** di ricerca.
...
1. Pochi predicati di ricerca non sono applicabili ai filtri di ricerca nel Brand Portal. Quando questi predicati di ricerca vengono pubblicati come parte del modulo di ricerca dall&#39;istanza AEM Author al Portale marchio, vengono esclusi. Di conseguenza, gli utenti possono vedere un numero minore di predicati nel modulo pubblicato sul Brand Portal. Consultate predicati di [ricerca applicabili ai filtri nel Portale](../using/brand-portal-search-facets.md#list-of-search-predicates)marchio.

1. Per Predicato opzioni, se un utente utilizza un percorso personalizzato per la lettura delle opzioni nell&#39;istanza AEM Author, non funzionerà nel Portale marchio. Tali percorsi e opzioni aggiuntivi non vengono pubblicati in Brand Portal con il modulo di ricerca. In questo caso, gli utenti possono selezionare l&#39;opzione **[!UICONTROL Manuale]** in **[!UICONTROL Aggiungi opzioni]** in Predicato **** opzioni per aggiungere manualmente queste opzioni in Brand Portal.
