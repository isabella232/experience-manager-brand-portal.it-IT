---
title: Uso del modulo schema metadati
seo-title: Uso del modulo schema metadati
...
uuid: 1a944a3b-5152-425f-b1ea-bfe331de928
...
# Uso del modulo schema metadati {#use-the-metadata-schema-form}
...
| [!UICONTROL Valutazione risorsa] | Valutazione media di una risorsa aggiunta da Risorse AEM prima che venga pubblicata sul Brand Portal. |
...
— [!UICONTROL `./jcr:content/metadata/dc:title`]: Memorizza il valore nel nodo di metadati della risorsa come proprietà [!UICONTROL `dc:title`].

— [!UICONTROL `./jcr:created`]: Visualizza la proprietà jcr sul nodo della risorsa. Se configurate queste proprietà sulle proprietà di visualizzazione, è consigliabile contrassegnarle come Disattiva modifica, in quanto sono protette. In caso contrario, quando salvate le proprietà della risorsa si verifica l’errore "Impossibile modificare le risorse".
...
* **[!UICONTROL Mostra campo vuoto in sola]** lettura: Contrassegnate questa proprietà per visualizzare una proprietà di metadati nella pagina Proprietà anche se non ha alcun valore. Per impostazione predefinita, quando una proprietà di metadati non ha alcun valore, non viene elencata nella pagina Proprietà.
...
>[!NOTE]
>
>Tutti i campi di metadati sono di sola lettura nell’editor di metadati di una risorsa. Poiché i metadati della risorsa devono essere modificati in Risorse AEM prima che una risorsa venga pubblicata nel Brand Portal.
...
Oltre ai moduli predefiniti, è possibile aggiungere moduli personalizzati per risorse di vari tipi MIME o creare un nuovo modulo con un tipo appropriato. Ad esempio, per aggiungere un nuovo modello per il sottotipo **[!UICONTROL immagine/png]** , creare il modulo sotto i moduli "immagine". Il titolo del modulo schema è il nome del sottotipo. In questo caso, il titolo è "png".
